...
type DeleteProductModalProps = {
  ...
  isOpen: boolean
  onClose: () => void
  onConfirm: (_combo: Combo) => Promise<void> | void
}
      ...
      <div className="w-full max-w-md rounded-2xl bg-background p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-foreground">Delete Combo</h2>
          </div>
        ...
        <p className="mt-5 text-sm leading-6 text-admin-muted">
          Are you sure you want to delete <strong className="font-bold text-foreground">{combo.name}</strong>?
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <Button type="button" variant="primary" size="sm" onClick={() => onConfirm(combo)}>
            Okay, Delete
          </Button>
        </div>
      </div>
